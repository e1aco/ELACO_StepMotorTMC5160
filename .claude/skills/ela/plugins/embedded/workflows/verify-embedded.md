...
```bash
python <BASE_DIR>/tools/build-dispatcher/scripts/builder.py \
  detect --project <项目目录>
```
...
```bash
python <BASE_DIR>/plugins/embedded/tools/flash-openocd/scripts/openocd_flasher.py --detect
```
...
```bash
python <BASE_DIR>/plugins/embedded/tools/flash-openocd/scripts/openocd_flasher.py \
  --artifact <产物路径> \
  --interface stlink \
...
```bash
python <BASE_DIR>/plugins/embedded/tools/serial-monitor/scripts/serial_monitor.py \
  --port COM5 \
  --baud 115200 \
  --duration 15 \
  --wait-reset \
...
```bash
python <BASE_DIR>/plugins/embedded/tools/serial-mcp/serial_monitor.py \
  --project "%CD%" --step "S<N>"
```
