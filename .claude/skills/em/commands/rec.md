...
| L0.5 知识库 | `<STATE_DIR>/require-index.md` | 总是（≤50 行，含 Require/ 索引） |
...
3. **【知识库加载】** 读 `<STATE_DIR>/require-index.md`（若存在）
   - 存在 → 注入索引内容作为 AI 背景知识（≤50 行）
   - 索引中包含 `.lnk` 核心文档的章节结构（开发进度+技术参数），AI 可快速定位
   - 不存在 → 跳过；若 `Require/` 目录存在但无索引 → 提示「运行 `/em req index` 生成索引」
4. **【旧版兼容】** state.md 不存在 → 读 `memory-log.md` 前 ~30 行（会话指纹+快速恢复信息+当前状态），跳过会话历史
5. **【嵌入式插件】** 若 `project.json.type == "embedded"` 或检测到 `.emv2/embedded/` → 加载 `plugins/embedded/PLUGIN.md`（仅文件名提示，不展开内容）
6. **【生成摘要】** 输出 5 行内的恢复摘要
