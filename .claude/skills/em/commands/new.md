...
5.7 **【子 Agent 并行调度评估】**（deep 档且子系统≥3 时）

   当 deep 档拆分出 ≥3 个子系统时，**brainstorm 阶段启用并行子 Agent**：

   1. 主 Agent 从 split.md 读取子系统列表
   2. 为每个子系统创建独立的子 Agent，并行调研技术方案
   3. 各子 Agent 返回：方案对比 + 风险评估 + 推荐
   4. 主 Agent 汇总 → 检测跨子系统冲突（引脚冲突、资源争用、时序依赖）
   5. 输出整合的 brainstorm.md

   详见 `docs/subagent-principles.md`

   **降级条件**：任一子 Agent 执行失败 → 主 Agent 自行处理该子系统（不阻塞其他）。
...
5.75 **【编码前准备】**（嵌入式项目时，合并 drv 决策 + Keil 预配置）
   ...
   **C. 编码前规范检查**（强制）
   - 生成任何代码前，**必须**完成以下检查：
     1. 加载 `references/ela_rules_quick.md`
     2. 确认目标项目的命名规范（函数/变量/类型/宏）
     3. 如有参考代码，**禁止原样复制** → 必须先按目标规范重命名后再使用
     4. **跨模块防撞**：新增宏/类型前，扫描目标模块已有定义，禁止同名不同值
     5. **elaco_main 不碰 HAL**：elaco_main.c 禁止直接 include HAL 外设头文件，必须通过 drv 层
     6. 输出检查结果：`✅ 编码前规范检查通过` 或列出需修正项
...
7. **【编码后质量关卡】**（强制，不可跳过）
   ...
   编码完成后、标记 done 之前，**必须**通过以下检查：

   **A. 14 项规范验证**
   - 逐条对照 `references/ela_rules.md` 第二十一节的 14 项检查
   - 输出审核报告，任一不通过则修正后重新检查
   - 参考格式：`📋 模块审核: <模块名> ... 结果: ✅ 通过（14/14）`

   **B. 注释同步检查**
   - 函数行为变更时，文档注释必须同步更新
   - 检查项：
     - `@ 说明` 是否反映当前实际行为
     - `@ 注意` 是否标注了 ISR 安全性、Flash 操作限制等
     - `@ 依赖` 是否标注了新增的跨模块依赖
   - 输出：`✅ 注释同步完成` 或列出需修正项

   **C. Flash/存储专项审查**（嵌入式项目 + 涉及 Flash/EEPROM 时）
   - 检查项：
     - 写入前是否擦除（Flash 只能 1→0）
     - 地址边界检查是否用 `>=`（防止越界）
     - 数据对齐（halfword 写入需 2 字节对齐）
     - 回读校验（写入后读回比对）
     - ISR 安全（Flash 操作期间 CPU 停顿，禁止在中断中调用）
     - 掉电安全（关键数据是否有冗余/校验）
   - 输出：`✅ Flash 存储审查通过` 或列出风险项

   **D. 禁止盲目复制**
   - 从其他版本/项目迁移代码时，**必须**先按目标项目规范重命名
   - 禁止原样搬运函数名、变量名、注释格式
   - 检查项：文件头注释、函数命名、变量命名、段标记是否符合目标规范
   - 输出：`✅ 代码适配完成` 或列出需修正项
