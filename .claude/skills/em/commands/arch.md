...
| 文件（新） | 文件（旧） | 触发条件 | 说明 |
|-----------|-----------|----------|------|
| `sessions/`（每会话独立） | `memory-log.md` | > 600行 或主步骤完成 | 会话历史，按月份归档到 `history/<YYYY>/<MM>/sessions/` |
| `project-spec.md` | — | S完成后 | 项目规格单，按里程碑归档 |
| `problem-log.md` | — | > 300行 | 问题追踪记录 |
| `decisions.md` | `decision-log.md` | > 300行 | 关键决策记录 |
...
- 会话结束自动归档
- 文件过大时自动归档
  - `sessions/` 或 `memory-log.md`（旧版）: > 600行
  - `project-spec.md`: S完成后
  - `problem-log.md`: > 300行
  - `decisions.md`（旧版 `decision-log.md`）: > 300行
- 手动命令归档: `/em arch`
