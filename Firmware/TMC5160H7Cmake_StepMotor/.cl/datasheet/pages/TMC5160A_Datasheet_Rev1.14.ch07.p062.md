...
TMC5160/TMC5160A DATASHEET (Rev. 1.14 / 2020-MAY-19)  62 
...
The StealthChop current regulator  imposes a lower limit for motor current regulat ion. As the c oil 
current can be measured in the shunt r esistor during chopper on phase  only, a minimum chopper 
duty cycle allowing coil current regulation is given by the blank time as set by TBL and by the 
chopper frequency setting. Therefore, the motor s pecific minim um coil current in StealthChop 
...
that in AT#1 the run current se t by the sense resistor, GLOBALSCALER and IRUN is well within the 
...
account all relevant settings. This lower current limit also applies f or modification of the motor 
