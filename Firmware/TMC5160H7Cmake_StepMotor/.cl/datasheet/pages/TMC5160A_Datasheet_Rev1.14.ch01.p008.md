...
TMC5160/TMC5160A DATASHEET (Rev. 1.14 / 2020-MAY-19)  8 
...
The motor  can optionally be  controlled by a st ep and direction input. In this case, the motion 
controller remains unused.  Active edges on the STEP input can be rising edges o r both rising and 
falling edges as controlled by another mode bit ( dedge). Using both edges cuts the toggle r ate of the 
...
internal table tran slates the counter value into the sine and cosine values which control the motor 
...
reduces motor standstill power consumption to zero, while still provi ding effective d ampening and 
...
TPOWERDOWN
power down 
delay timeRMS motor current trace
IHOLD
IRUN
IHOLDDELAY
power down 
