...
TMC5160/TMC5160A DATASHEET (Rev. 1.14 / 2020-MAY-19)  60 
...
depending on the actual motor velocity. Several additional factors influence the req uired voltage level 
...
controlled mode will not react to a change of the supply voltage or to events like a motor sta ll, but it 
provides very stable amplitude. It does not use nor require any means of current measurement. This 
is perfect when motor type and supply voltage are we ll known. Therefore, we recommend the 
...
supply voltage. Additi onally, operate in automatic tuning mode  in order to respond to parameter 
...
regulates the moto r current to the desired current setting. Automatic scaling is used as part of the 
...
velocity. Therefore, a high accele ration during AT#2 will require a higher setting of PWM_REG. With 
...
PWM_REG in phase AT#2 and the finished automatic t uning procedure (or non -automatic settings for 
PWM_OFS and PWM_GRAD) can be examined when monitoring  motor current during an accel eration 
phase Figure 7.5.  
