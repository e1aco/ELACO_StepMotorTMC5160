...
// TMC5160 SPI控制模式使用示例
// (SD=0, SPI=1, 模式1: Full Motion Controller)
...
void step1_init(void)
{
     ...
     */
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_IHOLD_IRUN,
                     (5 << 16) | (16 << 8) | 8);
}
 ...
 * ==========================================================
 *  CHOPCONF(0x6C) 关键位域:
 *    [27:24] MRES = 0(256微步) / 1(128) / ... / 8(全步)
 *    [16:15] TBL  = 0(16clk) / 1(24clk) / 2(36clk) / 3(54clk)
 *    [14]   CHM    = 0(SpreadCycle) / 1(固定关闭时间)
 *    [7:4]  HSTRT  = 斩波滞环开始(通常4~7)
 *    [2:0]  HEND   = 斩波滞环结束(通常0~3)
 *    [17]   -      = 保留,设为0
 *
 *  常用值 0x000101D5:
 *    MRES=0(256微步), TBL=1(24clk), CHM=0(SpreadCycle)
 *    TOFF=4, HSTRT=5, HEND=3, TPFD=4
 * ========================================================== */
void step2_chopper_config(void)
{
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_CHOPCONF, 0x000101D5);
}
...
void step3_position_move(void)
{
    ...
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_DMAX,    800);
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_TZEROWAIT, 10);

    /* ③ 写入目标位置 → 电机立即开始运动 */
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_XTARGET, 50000);
}
...
unsigned char step4_wait_for_position(void)
{
    ...
    do {
        status = Tmc5160_ReadReg(&g_tmc5160_chip1_st,
                                 TMC5160_RAMP_STAT);
...
/* ==========================================================
 *  第5步: 速度模式(连续旋转)
 *  说明: RAMPMODE=1(正转)或2(反转), VMAX就是目标速度 =0表示位置模式
 *  目标: 理解定位模式和速度模式的区别
 * ==========================================================
 *  注意:
 *    · 定位模式: VMAX是速度上限,目标由XTARGET决定
 *    · 速度模式: VMAX就是目标速度,持续转直到改RAMPMODE
 * ========================================================== */
 ...
 *  目标: 学会排查驱动故障
 * ==========================================================
 *  GSTAT(0x01):   bit0=复位 / bit1=驱动错误 / bit2=电荷泵欠压
 *  DRV_STATUS(0x6F): bits25:16=StallGuard负载值
 *                      bit31=过温 / bit30=过温预警
 * ========================================================== */
unsigned char step6_check_error(void)
{
    unsigned int gstat = Tmc5160_ReadReg(
        &g_tmc5160_chip1_st, TMC5160_GSTAT);

    if (gstat & 0x02) {         // bit1 = drv_err
        // 驱动错误! 需要读DRV_STATUS看具体原因
        unsigned int drv = Tmc5160_ReadReg(
            &g_tmc5160_chip1_st, TMC5160_DRVSTATUS);
...
/* 回零参数(需根据实际电机和机构调整) */
#define HOMING_VELOCITY     20000   // 回零速度(μsteps/s)
#define HOMING_THRESHOLD    500     // 堵转判定阈值(需实测)
#define HOMING_BACKOFF      2000    // 撞限位后退回距离(微步)
#define HOMING_TCOOLTHRS    500     // StallGuard使能阈值
...
void step7_homing_by_stall(void)
{
    ...
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_TCOOLTHRS, HOMING_TCOOLTHRS);

    /* ③ 先检查当前是否已经堵在限位上 */
    drv = Tmc5160_ReadReg(&g_tmc5160_chip1_st,
                          TMC5160_DRVSTATUS);
    sg = (drv >> 16) & 0x3FF;
    ...
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_RAMPMODE, 0);
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_XTARGET, HOMING_BACKOFF);
    step4_wait_for_position();
...
void spi_mode_demo_with_homing(void)
{
    ...
    /* ---- 定位运动(从零位开始走) ---- */
    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_XTARGET, 51200);  // 正转1圈
    step4_wait_for_position();

    Tmc5160_WriteReg(&g_tmc5160_chip1_st,
                     TMC5160_XTARGET, -51200); // 回到零位
    step4_wait_for_position();
