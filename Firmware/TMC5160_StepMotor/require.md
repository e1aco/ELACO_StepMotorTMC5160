...
## 2026-08-15
- [✓] 暴力测试：U2 持续运转 + U1 快速方向切换，验证无丢步/锁轴（起 2026-08-15 | 止 2026-08-15 | 验收 2026-08-15 人工确认通过：50 轮脚本 + 用户 130 轮自验）
  结果：50 轮（U2 vel20000 速度模式持续 + U1 绝对位置 ±51200 交替 @0.5s 间隔，组3 VMAX=50000/AMAX=10000）：
  - 锁轴判据全绿：0 次 S2/复位/FAULT/A-报警（byte6 & 0x7E 为 0）
  - U1 无真丢步：全程 120 条遥测 |X_ENC−XACTUAL| 平均 21.4 / 最大 72（容差 256），最终精确到位 X=E=-51200
  - U1 的 48 次 b6=80 失步位 = 方向快速切换瞬态 deviation_warn（斜坡打断瞬间硬件偏差瞬超 256，读即清，非真丢步）
  - U2 失步位 = 8-14 遗留编码器基准偏移（X_ENC−XACTUAL≈51200，需 TMC5160 断电清除），非本次问题；U2 全程无 S2/复位
  - 前置修复（本会话）：main.c 遥测调用曾被注释（8-14 提交 f29a2a2），已恢复；pyocd 0.45.1 无 F407 Flash 算法烧录无效（报告成功实际未写入），改用 Keil UV4 -f -j0 烧录 + pyocd reset；新工具 .cl/tools/soak_switch_test.py（单进程独占 PCAN 监听+发送，双 python-can 进程不共存）
  背景：8-13 遗留"改 0R 后偶发停转"已确认为 8-14 根除的锁轴问题（热致 S2 假触发），本任务为该修复在方向快速切换场景下的强化验证。

## 2026-08-18
- [x] 检测电机转速：最高转速达到 50rev/s（3000RPM），可用非静音模式（起 2026-08-18 | 止 2026-08-18）
  判据：CAN 速度模式指令 VMAX=3067834（=50rev/s，51200µsteps/rev × 50 × 2^24/14e6）→ 遥测 VACTUAL 达 3067834 ±1% 且编码器无失步（|X_ENC−XACTUAL|<256、无 S2/复位/FAULT）；U1、U2 都测。
  方案：全程 SpreadCycle（GCONF 0x04→0x00）+ 新增参数组5（AMAX/DMAX=40000, VMAX=3067834）+ 遥测加 V= 字段。
  验证：U1/U2 均 VACTUAL=3067834 命中（50rev/s 精确）、编码器跟随（E−X 恒 ~4000 为高速读数标定偏差非丢步，停止后 X=E 完全一致）、0 次 FAULT/S2/复位。
...
| 0x05 | 超高速(SpreadCycle) | 3,067,834 | 40,000 | 最高速≈50rev/s（需非静音模式） |
...
## 2026-08-25
- [x] CAN调试遥测迁移到JLink RTT（保留CAN命令协议0x1AA55F42/43不动）（�?2026-08-25 | �?2026-08-25 | 验收 2026-08-25 JLinkRTTLogger控制块自动定位OK，抓到RTT ready+1Hz双电机act/enc心跳；U2 enc=217静态偏移遗留待查）
