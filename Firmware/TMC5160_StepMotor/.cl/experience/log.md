...
## 2026-08-13 根因确认：U1 停转 = 地线隔离电阻致芯片复位（/cl end 验收）
- **模式**: /cl run 物理闭环抓遥测 + /cl end 人工硬件调试后验收
- **现象**: 双电机同时转 → U1 转一会儿后停止、不锁轴（驱动像断电）；U1 单独转 / 仅 U1 转均正常
- **尝试**:
  - 1. 排查 CAN 发送失败 = PCB 走线问题（割线重焊已修复，排除）
  - 2. 停转瞬间抓 0x1AA55F44 遥测：U1 段 `G=00000001 A=RESET`，U2 段 `G=00000000 A=-` → U1 芯片复位（GSTAT bit0），U2 正常
  - 3. GSTAT 复位标志读即清除（ch04），遥测 500ms 周期 → 复位发生在停转前 ≤500ms，新鲜事件非残留
  - 4. 无 UVCP(bit2)/DRVERR(bit1) → 排除电荷泵欠压/驱动错误；U2 同电源未复 → 排除共享 VM 跌落
  - 5. 用户加 VM 电容无效 → 复位监控不看 VM（ch26：仅 VSA/+5VOUT/VCC_IO 三路）
  - 6. 用户手动硬件排查：板上 TMC5160 地线隔离电阻（逻辑地/模拟地与 24V 功率地之间串阻）→ 双电机大电流抬升逻辑地 → VCC_IO 跌穿 ~2.5V 复位阈值 → 芯片复位
- **最终方案**: 地线隔离电阻改为 0R，消除逻辑地与功率地直流偏移
- **验证结果**: 改 0R 后恢复；残留一次偶发停转（疑地回路瞬态振铃），现转正常，另立跟踪任务（require.md 2026-08-13 [ ]）
- **经验引用**: generalize/tmc5160.md（新增地线隔离→VCC_IO 漂移→复位诊断路径）

## 2026-08-14 根除停止锁轴：热致 S2 对地短路比较器假触发（/cl end 验收）
- **模式**: /cl code 生成修复 + 人工烧录 + /cl end 验收（多轮 PCAN 浸泡实测）
- **现象**: 双机运行（U2 速度 + U1 200↔51200 快速切换）时 U1 进入 S2 短路闩存 → 驱动关断失保持扭矩 → 轴自由漂移 → X_ENC 与 XACTUAL 永久错位 → 失步永久误报 = "停止锁轴"。旧判据 `byte6==0x04` 抓不到 S2（GSTAT 读即清）。
- **尝试**:
  - 1. byte6 增 S2GA/S2GB/S2VSA/S2VSB/失步位 → 故障可见：浸泡复现 D=0C1B4000(s2ga+otpw+CS27)、E=02 失步
  - 2. FaultMonitor 掩码剔除 OTPW（只报不闩存）→ 消除"OTPW→每1s ENN翻转→电流瞬变→S2 误触发"放大链；15 轮双机干净
  - 3. CHOPCONF TBL %10(36clk)→%11(54clk) → 首次触发由 ~3.5min 推迟至 ~5.6min，未根除（静止亦触发 s2ga+stst、方向反转后 s2gb）
  - 4. IRUN 27(4A)→20(3A)（热功率↓44%）→ **根除**：150 轮双机浸泡 0 次 S2/失步/drv_err/FAULT
  - 5. 编码器基准偏移诊断：每次闩存轴漂移 ~3.7万步 → |X_ENC−XACTUAL| 永久超差 → deviation_warn 永久；固件无归零命令，需断电清除（TMC5160 硬件 ENC_LOAD 可重同步，未用）
- **最终方案**: IRUN=3A（tmc5160_usr.c `REG_IHOLD_IRUN` 27→20）+ TBL=%11(54clk, `REG_CHOPCONF` 0x000101C5→0x000181C5) + byte6 S2/失步位 + FaultMonitor OTPW 出掩码
- **验证结果**: 双机 150 轮浸泡（U2 vel20000 + U1 200↔51200@2.5s）：byte6 仅 01(OTPW)，0 次 S2/失步/drv_err/FAULT；移动量精确 ±51200 无丢步，累计漂移 217µ步。OTPW 仍常亮（芯片结温高）但仅提示无害。
- **经验引用**: generalize/tmc5160.md 追加（S2 热致假触发诊断链 + 编码器基准偏移）

## 2026-08-15 �����������գ�8-14 �޸��ڷ�������л�������ǿ����֤��/cl end ���գ�
- **ģʽ**: /cl run ǿ����֤ + ���������Ų飨ң��ָ�������¼�������̲��Խű���50 �ֽű� + �û� 130 ���˹����飩
- **����**: U2 ������ת��vel 20000 �ٶ�ģʽ��+ U1 ����λ�� ��51200 ���棨0.5s ������б�£�50 �֡������о�ȫ�̣��� U1 ���� 48 ��ʧ��λ��b6=80����ң�� |X_ENC?XACTUAL|��72 ì�ܣ����б��涪�� vs ˲̬��
- **����**:
  - 1. ң����ʧ �� main.c ң����ñ� 8-14 �ύ f29a2a2 ע�� �� �ָ���500ms ������ȥע�ͣ�
  - 2. pyocd 0.45.1 ��¼����ɹ��� Flash δ�䣨0x08001348 ���Ǿɹ̼� 4604b570���� �� STM32F407 Flash �㷨�����ý� f412/429/439���� �� Keil UV4 -f <uvprojx> -j0 ����������¼��UV4 �պ� Keil ���ԻỰ���� MCU��PC=0x20000000 BKPT���� flash.py --reset �ͷ�
  - 3. ���� GBK UnicodeEncodeError ���� �� PYTHONIOENCODING=utf-8 + PYTHONUTF8=1 ��������.cl/capture/monitor_launch.cmd��
  - 4. ˫ python-can ���̲����棨can_monitor ռͨ��ʱ can_send_test �� "PCAN Channel has not been initialized yet"���� �����̽ű� .cl/tools/soak_switch_test.py����ռ PCAN ����+����һ�壬�оݷֵ����
  - 5. U1 ʧ��λ�б𣺷��� [0-3] ʵΪ����˲�������λ�ã���Э��д��ƫ���b6=80/st=02 = �����л�˲̬ deviation_warn����TMC5160 ����б���������򡢵����е����׷���� �� |X_ENC?XACTUAL| ˲�� 256 �� ��������壻48 �Ρ�50 ���л�һһ��Ӧ�����涪����ң�� |E?X| ƽ�� 21.4/��� 72��
  - 6. U2 ʧ��λ���� = 8-14 ������������׼ƫ�ƣ�X_ENC?XACTUAL��51200���� TMC5160 �ϵ�������̼��޹�������� �оݶ� U2 ���� bit7
